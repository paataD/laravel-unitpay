...
'providers' => [
    ...
    PaataD\UnitPay\UnitPayServiceProvider::class,
...
```php
    'UnitPay' => PaataD\UnitPay\Facades\UnitPay::class,
```

Publish the configuration file and views
``` bash
php artisan vendor:publish --provider="PaataD\UnitPay\UnitPayServiceProvider" 
```

Publish only the configuration file
``` bash
php artisan vendor:publish --provider="PaataD\UnitPay\UnitPayServiceProvider" --tag=config 
```

Publish only the views
``` bash
php artisan vendor:publish --provider="PaataD\UnitPay\UnitPayServiceProvider" --tag=views 
```
...
- [PaataD](https://github.com/actionm)
